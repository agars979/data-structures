...
var tip = d3.tip()
  ...
  .offset([-10, 0])
  .html(function(d) {
    return "<strong>Window closed</strong> <span style='color:red'>" + formatPercent(d.num_obs) + "</span>";
  })
...
app.get('/aameetings', function(req, res) {
    ...
    var thisQuery = `SELECT address, title as location, json_agg(json_build_object('day', day, 'time', startTime)) as meetings
                 FROM aameetings 
                 WHERE day = 'Fridays' and startTime = '7:00 PM' 
...
app.get('/aa', function(req, res) {
    ...
    // SQL query 
    var thisQuery = `SELECT lat, long, json_agg(json_build_object('title', title, 'address', address, 'from', startTime, 'to', endTime, 'day', day, 'typeOfMeeting', meetingType)) as meetings
                 FROM aameetings 
                 WHERE day = 'Fridays' and startTime = '7:00 PM' 
                 GROUP BY lat, long
                 ;`;
